...
barley_all <-  barley %>% select(7:103) %>%
  select(-N_Efficiency, -CropType_Long, -Variety, -Year,
...
folds <- createFolds(barley_all$RP_Go, k = 5, list = TRUE, returnTrain = TRUE)
...
for (fold_index in 1:length(folds)) {
  ...
  model <- ctree(RP_Go ~ ., data = fold_train_data, controls = ctree_control)
  
  pred <- predict(model, newdata = fold_test_data)
  
  predictions[[fold_index]] <- pred
  actual_values[[fold_index]] <- fold_test_data$RP_Go
  
  fold_r[fold_index] <- cor(pred, fold_test_data$RP_Go)
}
