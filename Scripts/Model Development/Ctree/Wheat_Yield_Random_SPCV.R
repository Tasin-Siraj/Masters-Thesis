...
for (i in 1:k) {
  ...
  p <- p +
    geom_point(data = wheat_all[test_indices, ], aes(x = Longitude, y = Latitude), color = "firebrick2", size = 2) +
    geom_point(data = wheat_all[train_indices, ], aes(x = Longitude, y = Latitude), color = "darkblue", size = 2)
...
combined_plot <- combined_plot + 
  plot_annotation(title = "Random Spatial Locations Across Folds for Winter Wheat")
combined_plot

ggsave("Wheat_Random_Spatial.png", combined_plot, width = 16, height = 9, units = "in", dpi = 300)
