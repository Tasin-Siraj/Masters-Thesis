...
spatial_folds <- ggplot() +
  geom_sf(data = germany, fill = "white", color = "gray50") +
  geom_sf(data = fold_1, aes(color = "Cluster 1"), size = 2, alpha = 0.6, show.legend = TRUE) +
  geom_sf(data = fold_2, aes(color = "Cluster 2"), size = 2, alpha = 0.6, show.legend = TRUE) +
  geom_sf(data = fold_3, aes(color = "Cluster 3"), size = 2, alpha = 0.6, show.legend = TRUE) +
  geom_sf(data = fold_4, aes(color = "Cluster 4"), size = 2, alpha = 0.6, show.legend = TRUE) +
  geom_sf(data = fold_5, aes(color = "Cluster 5"), size = 2, alpha = 0.6, show.legend = TRUE) +
  labs(title = "Winter Soft Wheat", x = "Longitude", y = "Latitude") +
  scale_color_manual(values = c("Cluster 1" = "firebrick2", 
                                "Cluster 2" = "blue", 
                                "Cluster 3" = "green", 
                                "Cluster 4" = "magenta", 
                                "Cluster 5" = "purple"),
...
spatial_folds
ggsave("Wheat_SPCV.png", spatial_folds, width = 16, height = 9, units = "in", dpi = 300)
...
for (fold in 1:length(cluster_folds$splits)) {
  ...
  fold_r[fold] <- cor(pred, test_data$Ert_dt_ha)
}
...
plot(predictions[[4]], actual_values[[4]])
plot(predictions[[5]], actual_values[[5]])
