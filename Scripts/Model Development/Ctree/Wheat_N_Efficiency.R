...
folds <- createFolds(wheat_all$N_Efficiency, k = 5, list = TRUE, returnTrain = TRUE)
...
for (fold_index in 1:length(folds)) {
  ...
  model <- ctree(N_Efficiency ~ ., data = fold_train_data, controls = ctree_control)
  
  pred <- predict(model, newdata = fold_test_data)
  
  predictions[[fold_index]] <- pred
  actual_values[[fold_index]] <- fold_test_data$N_Efficiency
  
  fold_r[fold_index] <- cor(pred, fold_test_data$N_Efficiency)
}
