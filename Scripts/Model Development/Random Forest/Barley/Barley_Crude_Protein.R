...
set.seed(42)
model <- caret::train(RP_Go ~ ., data = data_train, method = "rf", trControl = cross_val)
pred <- predict(model, data_test)
r <- cor(pred, data_test$RP_Go) %>% round(., 4)
r

set.seed(42)
var_imp <- vip(model, method = "permute", train = data_train, target = "RP_Go", metric = "rsq",
