...
process_strategy <- function(data, strategy_name) {
  baseline_data_strategy <- data %>%
    ...
    select(Attempt, baseline_value_strategy = N_Efficiency)
  
  wheat_RC_strategy <- data %>%
    left_join(baseline_data_strategy, by = "Attempt") %>%
    mutate(RC_efficiency_strategy = (N_Efficiency - baseline_value_strategy) / baseline_value_strategy) %>%
...
for (strategy in unique_strategies) {
  ...
  result_no_na <- na.omit(result$RC_efficiency_strategy)
...
SP_efficiency <- strategy_performance %>%
  rename(Probability_of_better_efficiency = Relative_Performance_Better)


SP <- merge(SP_Yield, SP_protein, by = "strategy")
SP <- merge(SP, SP_efficiency, by = "strategy")
