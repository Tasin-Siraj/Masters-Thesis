...
process_strategy <- function(data, strategy_name) {
  baseline_data_strategy <- data %>%
    ...
    select(Attempt, baseline_value_strategy = RP_Go)
  
  wheat_RC_strategy <- data %>%
    left_join(baseline_data_strategy, by = "Attempt") %>%
    mutate(RC_protein_strategy = (RP_Go - baseline_value_strategy) / baseline_value_strategy) %>%
...
for (strategy in unique_strategies) {
  ...
  result_no_na <- na.omit(result$RC_protein_strategy)
...
SP_protein <- strategy_performance %>%
  rename(Probability_of_better_Protein = Relative_Performance_Better)
